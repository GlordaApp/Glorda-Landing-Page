  ...
  return (
    <section className="py-24 bg-[#FDF8F5] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {/* Desktop: horizontal steps — direction handles order automatically */}
        <div className="hidden lg:flex items-start justify-between relative gap-4">
          {/* Connecting dashed line */}
          <div className="absolute top-10 start-[10%] end-[10%] h-px z-0">
          ...
          {steps.map((step, i) => (
            <motion.div key={i} initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay: i*0.12 }}
              ...
              <div className="w-20 h-20 rounded-full bg-primary shadow-lg shadow-primary/30 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 relative">
                {stepIcons[i]}
                <div className="absolute -top-1.5 -end-1.5 w-6 h-6 rounded-full bg-secondary text-white text-xs font-extrabold flex items-center justify-center shadow-sm">
                  {i + 1}
                </div>
              </div>
        ...
        {/* Mobile: vertical steps — direction handles alignment */}
        <div className="flex lg:hidden flex-col gap-0 relative">
          {/* Vertical line at inline-start */}
          <div className="absolute top-0 bottom-0 w-px bg-primary/20 start-10" />
          {steps.map((step, i) => (
            <motion.div key={i} initial={{ opacity:0, x: -20 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }} transition={{ delay: i*0.1 }}
              className="flex gap-5 pb-8 relative z-10">
              <div className="w-20 h-20 rounded-full bg-primary shadow-md shadow-primary/30 flex items-center justify-center flex-shrink-0 relative">
                {stepIcons[i]}
                <div className="absolute -top-1 -end-1 w-5 h-5 rounded-full bg-secondary text-white text-[10px] font-extrabold flex items-center justify-center">
                  {i + 1}
                </div>
              </div>
